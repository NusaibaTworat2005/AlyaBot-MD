...
const jsonPath = path.join(process.cwd(), 'lib', 'stellar.json')

let conn = null
if (!global.termux) {
  conn = new Database(dbPath, { fileMustExist: false, timeout: 10000 })
...
global.loadDatabase = function loadDatabase() {
  ...
  if (global.termux) {
    if (fs.existsSync(jsonPath)) {
      try {
        const raw = JSON.parse(fs.readFileSync(jsonPath))
        global.db.data = raw
      } catch {}
    }
  } else {
...
global.saveDatabase = function saveDatabase() {
  ...
  if (global.termux) {
    fs.writeFileSync(jsonPath, JSON.stringify(global.db.data, null, 2))
  } else {
